...
The drill now toggles each clicked circle independently between shared ticks. Switching Magic → Ranged can show both circles; clicking still-lit Magic again clears its local circle while the sequence of orders selects Magic for the next protection check. At the next tick the display reconciles to the single selected protection. Accessible pressed states match the visible circles; the overhead and Active label show committed protection. The previous implementation kept overlapping highlights but incorrectly used the selected protection to decide which local circle to clear on repeat clicks.

This visual overlap is useful for one-tick alternating and does not grant simultaneous protection from two styles. Single-prayer conservation still requires an off–on pair between boundaries, as distinguished in [RuneLite's prayer flick guidance](https://github.com/runelite/runelite/wiki/Prayer). Our deterministic 600 ms reconciliation models the tick boundary, not network round-trip latency or packet arrival timing; idle controls remain immediate because no drill clock is running.
