...
The drill keeps independent local toggle bits and a separate committed protection. Pointer-down toggles the clicked icon immediately; conflicting icons remain lit until tick reconciliation. Explicitly pressing a lit icon clears that local bit immediately, while overhead protection still changes on a game tick. Accessible pressed states match the visible circles. The temporary extra display hold that merged overhead protection into the icon highlights was removed after the SDK comparison below; it hid part of the SDK's click feedback.
...
## Colosim SDK input and audio comparison

Compared the current OldSchoolSDK source at `04fdaee3d155238e54cf16c1ac259f6c2b210078`, and opened the linked Inferno simulator in Chrome. Its [ClickController](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/ClickController.ts) handles mouse-down; [PrayerControls](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/controlpanels/PrayerControls.ts) immediately calls the clicked prayer's toggle. Our old DOM click handler ran on release, so holding a press across a tick could put the input on a different tick from the SDK. Prayer input now uses primary mouse/pen presses and every touch-down, retaining detail-zero click activation for keyboards and assistive tools without a second toggle on pointer release.

[BasePrayer](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/BasePrayer.ts) maintains independent local highlights, pending protection, and per-prayer on/off sound flags. [World](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/World.ts) runs player pre-tick prayer resolution before NPC attack processing. [Player.activatePrayers](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/Player.ts) then emits sound flags during the player movement step, on the world tick. Sounds follow prayer-list order, off before on. Our prior audio played at click time instead, and our added highlight hold obscured explicit off/on feedback. Both now follow this SDK ordering.

The browser audio context is unlocked/decoded on the press, but playback waits for the shared tick. Repeated inputs coalesce into one off and one on flag per prayer per tick. Pause holds pending sounds; reset discards them and invalidates delayed playback. The SDK's configured input delay applies to queued world actions; prayer-panel presses bypass that queue, so no artificial prayer latency was added. We retain the fixed 600 ms deadlines and the trainer's long-stall pause policy rather than importing the whole rendering/game engine. This aligns the observed input and feedback contract, not every part of Colosim's combat simulation.

## Making the circle boundary visible

The user's follow-up describes the previous prayer clearing immediately or too soon after switching. This visual symptom is reproducible with a press near the existing tick boundary: a press 550 ms into a tick leaves only 50 ms of overlap. In the sampled reference-video sequence, short single-circle gaps are followed by sustained overlap because the next press follows the clearing boundary. The current behavior does not guarantee 600 ms of overlap from each click.

The pre-start interface previously ran its prayer clock while leaving its timing meter stationary. The meter directly below the prayer book now reads the actual tick deadline before, during and after a run. The duplicate arena meter was subsequently removed from prayer drills; blowpipe practice retains its own meter because it has no prayer book. The idle scheduler uses absolute deadlines too; the meter paints the remaining interval without driving prayer logic. Pausing freezes its display and reduced-motion preferences use four progress steps. The lesson explains why a late switch gives a short overlap. The user subsequently reported that alternating works much better.

## Touch prayer input

Reproduced ignored prayer presses when a second finger landed while the first was still touching the screen. The original primary-pointer filter accepted mouse input but discarded additional touch contacts; [PointerEvent.isPrimary](https://developer.mozilla.org/en-US/docs/Web/API/PointerEvent/isPrimary) distinguishes the first active touch from the others. Prayer buttons now accept each touch-down while retaining primary-button filtering for mouse and pen. Releases still do not toggle again.

Also reproduced page scrolling from slight finger movement after a prayer press. The prayer buttons use [`touch-action: none`](https://developer.mozilla.org/en-US/docs/Web/CSS/touch-action) and suppress selection/callouts so a control press remains a control gesture. This applies only to the protection buttons; swipes beginning on unused book artwork and the surrounding page remain ordinary scrolling. The timing and single committed protection are unchanged.
