...
The pre-start interface previously ran its prayer clock while leaving its timing meter stationary. Added a meter directly below the prayer book, and made both meters read the actual tick deadline before, during and after a run. The idle scheduler uses absolute deadlines too; the meters paint the remaining interval without driving prayer logic. Pausing freezes their display and reduced-motion preferences use four progress steps. The lesson now explains why a late switch gives a short overlap. This addresses an invisible timing cue; it does not establish that every part of the user's reported difference has been reproduced.
