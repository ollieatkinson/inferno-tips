...
## Complete-mechanic scoring and longer challenges

- Mager and bat lazy flicks, blob reads, supplies, one-tick conservation, combined blob/mager movement and blowpipe walking now score complete sequences. Diagnostic checks remain in the review without granting partial points. Holding a prayer, skipping supplies, standing still or leaving the blowpipe on auto-attack cannot earn points for the omitted mechanic. Flick & Move keeps its agreed 8/9 target.
- Food ends on tick 37 and potions on tick 33, allowing each last supply gap to be checked against the next incoming attack. Live rules, pass targets, guided forecasts, result labels and progress use the revised scoring. Old scores/passes are retained as earlier-scoring references; new bests and passes start separately while attempt counts survive. Resetting saved progress also clears circuit records.
- Added Hard circuit and Endless gauntlet to Practice drills, with reloadable routes. Both use three lives, normal 600 ms ticks, hidden hints, automatic stages and local personal bests per mode. Hard has five stages; Endless adds a mager warm-up, progresses through the same five stages, then repeats movement with a blob/mager, double blobs and triple Jad. Difficulty and point values cap at this final tier; speed does not increase.
- A correct scored beat earns the current stage value only if every scored check on that beat succeeds. Multiple failures on the same tick cost one life. Diagnostic checks do not charge lives or grant points. Three misses end the run. Manual Finish banks the current score, and the hard finale ends automatically. Paused/hidden/stalled runs stay playable as practice without recording a high score or individual drill passes. Each new stage discards its completed engine history, so an endless run retains bounded encounter state.
- All 121 unit tests pass, covering ideal patterns, bad-strategy rejection, last-supply protection, migration, stage progression, grouped lives, duplicate/stale callbacks, 300 endless stages, score validation and pause exclusions. Astro check/build passes without diagnostics. Existing browser checks pass for all eight revised drills, the unchanged Flick & Move target, guided restart-to-challenge, specific failure feedback and site settings persistence.
- Chrome DevTools desktop play completed the first hard stage on the real clock with 360 points and all three lives; mobile/touch emulation confirmed the score/retry panel, no horizontal overflow and no console warnings/errors. Stage cue/arena dimensions remain consistent to keep prayers in place as encounters change. Mobile retry checks explicitly bring the button into view before comparing positions, avoiding a false failure from Playwright's own click scrolling. Physical-device testing has not been performed.
- Seven production browser tests pass for circuit discovery, three-life failure, retry, stage progression and score carry-over, personal-best persistence/separation, paused-run exclusion, a full five-stage 3,510-point hard clear, mobile stage/finish/retry stability, and old-score/reset behavior.

## Survival death screen

- Losing the final life in Hard or Endless now opens a viewport-covering native dialog. A darkened, desaturated game sits behind an animated red serif “YOU DIED” title. Score, stage reached, encounter and browser personal best stay visible wherever the player has scrolled. Practice and unavailable-storage states use their own record messages.
- Retry and Back to drills require an explicit choice, with a brief input guard against the last prayer tap becoming a retry. The background is inert, tab focus stays inside, and prayer-tab shortcuts cannot act behind the dialog. Reduced-motion preferences disable the entrance animation. Scroll locking preserves the layout; closing before remounting the trainer avoids native-dialog focus restoration scrolling Retry back to the top. Manual Finish and successful clears do not show a death screen.
- Seven targeted production browser checks pass: death/retry discovery, normal stage progression and manual finish, paused/manual finish, mobile retry stability, desktop/mobile death stats and keyboard containment, repeat death/exit, and practice death without a saved high score. Astro check/build passes without diagnostics.
- Chrome DevTools screenshots at 1280×1000 and touch-emulated 390×844 confirm the overlay and buttons fit, with no horizontal overflow or console warnings/errors. Death preserved scroll positions at 713 px desktop and 1285 px mobile. No physical-device test was performed.

## Compact challenge timing

- Challenge, Hard and Endless replace the filling tick meter with one small cycle digit beside the prayer controls (beside the weapon controls for blowpipe). The full run tick counter, beat-dot row and mager countdown/phase strip are omitted in challenge mode. Guided practice retains its meter, full counter and attack cues.
- The number shows the last registered cycle beat, wraps at the encounter's period and stays still while paused. Movement drills use 1–4 to match their tile deadline. Before the first beat it shows a dash. An accessible timer label describes the cycle without announcing every tick. Prayer reconciliation, overheads, scoring and the 600 ms scheduler are unchanged.
- All 121 unit tests and the production build pass. Fifteen distinct production browser checks passed across batches: seven compact-display cases, guided mager and idle-clock behavior, desktop/mobile start and finish/retry stability, a complete mobile alternating run with overlapping fingers, and a real-clock 36-tick challenge. Fake-clock tests now advance successive game ticks separately, allowing React's timer effects to settle without depending on the removed meter's animation frames.
- Chrome DevTools touch emulation at 390×844 confirms one compact digit, no challenge tick meters, no horizontal overflow and no console warnings/errors.

## Practice result explanations

- Completed individual drills show the score against the practice target and group the actual failed requirements, with affected tick counts, an example and a drill-specific correction. The most frequent issue appears beside Retry. Guided and challenge results share this report; circuits retain their existing stage/death feedback.
- Diagnostic requirements are included even when only complete rounds score. Aggregate failures are excluded, and simultaneous checks in one category count as one affected tick. The report does not infer precise early/late click timing from boundary-only observations. Existing alternating phase detection is shared with the report.
- All 130 unit tests and Astro check/build pass. Production browser checks at 1280px and 390px verify a held-Magic guided run reports zero completed lazy-flick rounds and 27 quiet-tick mistakes, keeps scroll position at completion and removes old advice on retry.
- Chrome DevTools mobile emulation at 390×900 confirms readable advice beside Retry after a real-time run, no horizontal overflow and no console warnings/errors.

## Cloud high scores

- Added the Worker/D1 API, server-issued seeded sessions, bounded stage uploads and server replay through the same drill/series engines. Guest cookies are HttpOnly; names are public labels. Publishing uses server-validated Turnstile. Public boards separate modes, scoring versions and Monday-UTC weeks, select one best per guest and share ranks for equal performances.
- Completed runs can be published from death or normal completion screens. Pending uploads survive navigation/reload; local-only runs remain available during outages. Cloud requests run outside the game clock. Ranked retries preserve the death screen while obtaining a new session and only close it when the next run is ready. Mobile navigation accommodates the fifth item without a second row.
- 137 unit tests pass, including full Hard and ten-stage Endless replay parity, partial/dead runs, malformed input, week boundaries and pending upload recovery. Seven API integration tests pass in Miniflare/workerd with real D1: recomputation, idempotency/concurrency, ownership/origin rejection, elapsed time, practice/name/Turnstile rejection, rank ties, weekly/version separation, removal/blocking and scheduled retention cleanup.
- Four new production browser tests pass for desktop/mobile death-screen saving and save retries, reload recovery, local fallback and disabled-backend messaging. All ten existing circuit/death regressions passed across batches, including a full Hard clear. The repeated mobile death test now waits for modal mount before advancing its fake clock through the deliberate 700 ms input guard.
- Chrome DevTools at 390×900 completed a real-clock Hard attempt against the local Worker, which recomputed 20 points; the actual Turnstile test widget verified and the score was published into local D1, then displayed as this browser's leaderboard entry. The compact verification widget fits the narrow controls panel. No horizontal overflow or console warnings/errors on the final board view.
- Astro check/build and production Worker deployment dry-run pass. Staging and production configurations remain disabled with placeholder D1 IDs. `wrangler whoami` reports no authentication in this workspace, so no cloud databases, widgets or live Worker deployments were created. Deployment and moderation steps are documented in `worker/README.md`.

## Cloud provisioning and staging rollout — 20 September 2026

- Authenticated Wrangler and provisioned separate Western Europe D1 databases, API Workers and managed Turnstile widgets for staging and production. Applied `0001_scores.sql` remotely to both databases and stored verification secrets with Wrangler. Secrets are not committed. Both environments disable the localhost-only test-key fallback.
- Staging serves the built frontend and `/api/v1/*` from `https://inferno-tips-api-staging.oliveratkinson.workers.dev`. This keeps cookies on one origin without requiring a second Pages project or DNS record. The staging deploy script now builds before uploading assets. Production keeps the existing Pages frontend, with `inferno.tips/api/v1/*` routed to its Worker.
- Staging is enabled. Chrome DevTools mobile emulation completed a real-clock Hard attempt against the remote Worker; D1 replay returned 20 points, stage 1, not practice. The score form and compact live Turnstile widget fit at 390 px without horizontal overflow. The live leaderboard returned HTTP 200; completing the run without the owner's cookie returned 401, and publishing with a dummy token returned 400. No test score was published.
- Both widgets have the expected domain restriction, both deployed Workers list `TURNSTILE_SECRET`, and Siteverify rejects dummy tokens using the real widget credentials. A successful live Turnstile token has not been obtained: the automated Chrome session failed the live challenge. Cloudflare documents that automated browsers are detected as bots; the local full publication flow already passed with official test keys. A normal-browser staging save was requested before enabling production; this is still outstanding.
- Production `/api/v1/config` responds with the production site key and `enabled: false`. Its migrated database has no runs or scores. The pre-enable D1 Time Travel bookmark is `00000002-00000002-000050ec-bd7fc3db3a6ed525005425ecacd32789`; restoration must specify `--config worker/wrangler.jsonc --env production` as well as the bookmark. Production remains disabled pending the live save check.

## Production high scores enabled — 20 September 2026

- The user's normal-browser staging save passed live Turnstile verification: `Olbo`, 120 points, Hard stage 1. Confirmed the persisted score in staging D1 and rank 1 in the public leaderboard response. The staging entry remains in its isolated database.
- Enabled production `CLOUD_ENABLED` and deployed Worker version `6e60de5f-07cb-42b0-ae98-8121757619ba`. The production config reports enabled with its real site key; Hard all-time and Endless weekly boards both return HTTP 200. Chrome DevTools confirms the High scores page renders its empty-board state correctly.
- Completed a real-clock Hard smoke run through the production frontend: the remote API recomputed 20 points, stage 1, not practice, and the save control appeared. Publishing with a dummy Turnstile token returned HTTP 400. No public production test score was created. The only console error was the expected HTTP 400 from the deliberate invalid-token check; no application exceptions were reported.
- Live successful publication was verified on staging by the user; production checks cover run recording, server scoring, leaderboard reads and verification rejection. A successful production human-verification submission was not repeated in the automated browser. No application/scoring code changed during enablement; existing implementation tests and the rollout build remain applicable.
