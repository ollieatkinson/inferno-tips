...
Run `npm run db:local`, `npm run dev:api` and `npm run dev` to try the complete flow at `http://localhost:4321`. Staging is deployed at https://inferno-tips-api-staging.oliveratkinson.workers.dev. Production resources are provisioned, with public submissions disabled pending live verification. See [Worker setup, endpoints, deployment and moderation](worker/README.md). Accounts and progress sync are follow-up features.
