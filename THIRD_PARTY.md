...
- Highlight timing follows the client-side per-prayer toggle and tick reconciliation shown in [OldSchoolSDK's BasePrayer](https://github.com/OldSchoolSDK/osrs-sdk/blob/04fdaee3d155238e54cf16c1ac259f6c2b210078/src/sdk/BasePrayer.ts). Activating another prayer leaves the old circle lit until the next game tick; local clicks toggle their own bits, while the drill keeps its committed protection visibly lit for the full tick, including through an off–on pair. See `docs/research.md` for the client reference and video comparison. Visual circles do not grant simultaneous protection or change the drill's queued inputs.
